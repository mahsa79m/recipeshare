@if(Auth::check())
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t sm:hidden z-40 shadow-lg" dir="rtl">
        <nav class="flex justify-around items-center h-16">
            ...
            <a href="{{ route('users.show', Auth::user()) }}" class="flex flex-col items-center text-gray-600 hover:text-amber-600 transition {{ request()->routeIs('users.show', Auth::user()) ? 'text-amber-600' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
