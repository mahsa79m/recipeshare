    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-1 transition-all duration-300 group">
        <a href="{{ route('recipes.show', $recipe) }}">
            ...
            <div class="relative h-48">
                <img src="{{ $recipe->image_path ? asset('storage/' . $recipe->image_path) : 'https://placehold.co/400x300/F5F5F5/333333?text=No+Image' }}" alt="{{ $recipe->title }}" class="w-full h-full object-cover">
            ...
            <div class="p-4 flex flex-col flex-grow">
                ...
                <div class="flex items-center mt-auto pt-2 border-t border-gray-100">
                    <img class="h-8 w-8 rounded-full object-cover" src="{{ $recipe->user->profile_image_path ? asset('storage/' . $recipe->user->profile_image_path) : 'https://ui-avatars.com/api/?name=' . urlencode($recipe->user->name) }}" alt="{{ $recipe->user->name }}">
                    <div class="mr-2">
...
@empty
    {{-- این بخش خالی می‌ماند چون پیام "نتیجه‌ای یافت نشد" در ویو اصلی مدیریت می‌شود --}}
@endforelse
